...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
    ...
    for (int i = 0; i < count; i++) {
         ...
         */
        ip_hdr->version = 4;    //  versiunea de IP = 4; IPV4
        ip_hdr->ihl = 5;    //  dimensiunea in cuvinte de 32 de biti a headerului
        ip_hdr->tos = 0;
        ...
        memcpy(&ip_hdr->daddr, &ip_addr.s_addr, 4);
        ...
        if (ricmp_hdr->code == 0 && ricmp_hdr->type == 0) {
            printf("Reply after: %fms ttl: %d seq: %d\n", time_elapsed, rip_hdr->ttl, ricmp_hdr->un.echo.sequence);
        }
    }
}
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
    ...
    for (int i = 0; i < 64; i++) {
        ...
        if (ricmp_hdr->code == 0) {
            if (ricmp_hdr->un.echo.sequence <= 0 || rip_hdr->ttl <= 0) {
                unsigned int ip = rip_hdr->saddr;
                unsigned char bytes[4];
                bytes[0] = ip & 0xFF;
                bytes[1] = (ip >> 8) & 0xFF;
                bytes[2] = (ip >> 16) & 0xFF;
                bytes[3] = (ip >> 24) & 0xFF;
                printf("ip:%d.%d.%d.%d ", bytes[0], bytes[1], bytes[2], bytes[3]);
                printf("time:%fms\n", time_elapsed);
            }
