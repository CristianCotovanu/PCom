...
//        printf("My checksum: %d\n", ip_hdr->check);
		...
		/* TODO 4: Check TTL >= 1 */
        if (ip_hdr->ttl < 1) {
            printf("TTL a ajuns la 0\n");
            continue;
        }

		/* TODO 5: Find best matching route (using the function you wrote at TODO 1) */
        struct route_table_entry* best_matching_route = get_best_route(ip_hdr->daddr);

        if (best_matching_route == NULL) {
            printf("Pachet imposibil de gasit\n");
            continue;
        }
        ...
        struct arp_entry* best_matching_arp = get_arp_entry(best_matching_route->next_hop);

        if (best_matching_arp == NULL) {
            printf("ARP imposibil de gasit\n");
            continue;
        }

        memcpy(eth_hdr->ether_dhost, best_matching_arp->mac, 6);
        get_interface_mac(best_matching_route->interface, eth_hdr->ether_shost);

		/* TODO 8: Forward the pachet to best_route->interface */
		send_packet(best_matching_route->interface, &m);
	}
}
